...
class ResBlockEDSR(nn.Module):
    def __init__(self, num_filters=128):
        super(ResBlockEDSR, self).__init__()
        self.conv1 = Conv2d(num_filters, num_filters, 3, stride=1)
        self.conv2 = Conv2d(num_filters, num_filters, 3, stride=1)
        self.conv3 = Conv2d(num_filters, num_filters, 3, stride=1)

    def forward(self, input):
        conv1 = nn.ReLU()(self.conv1(input))  # 128 / 1
        conv2 = nn.ReLU()(self.conv2(conv1))  # 128 / 1
        conv3 = (self.conv3(conv2))  # 128 / 1
        return input + conv3
...
class SuperResolution(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, num_filters=64):
        ...
        self.rb1 = ResBlockEDSR(num_filters)
        self.rb2 = ResBlockEDSR(num_filters)
        self.rb3 = ResBlockEDSR(num_filters)
        self.convout = ConvTranspose2d(num_filters, out_channels, 3, stride=2)

    def forward(self, x_hat_ds):
        conv1 = self.conv1(x_hat_ds)  # 128 / 1
        rb1 = self.rb1(conv1)  # 128 / 1
        rb2 = self.rb2(rb1)  # 128 / 1
        rb3 = self.rb3(rb2)  # 128 / 1
        x_hat_sr = self.convout(rb3)
