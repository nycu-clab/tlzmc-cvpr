...
from torch_compression.torchac.torchac import ac
...
def estimate_bpp(likelihood, num_pixels=None, input=None, mask=None, likelihood_bound: float = 1e-9):
...
class EntropyModel(nn.Module):
    ...
    quant_modes = ["noise", "universal", "round", "UQ", "pass", "RUN"]
...
class AdaptiveSymmetricConditional(EntropyModel):
    """Symmetric conditional entropy model (base class).
    ...
    def __init__(self, use_mean=False, scale_bound=None, use_quant=True, **kwargs):
        super(AdaptiveSymmetricConditional, self).__init__(**kwargs)
        self.use_mean = bool(use_mean)
        self.use_quant = bool(use_quant)
        self.condition_size = 1 + int(self.use_mean)
        self.idxmin = np.log(self.SCALES_MIN)
        self.idxmax = np.log(self.SCALES_MAX)
    ...
    def _set_condition(self, condition, quant=None):
        assert condition is not None, f'{self.__class__.__name__} should given condition'
        assert condition.dim() == 4 and condition.size(1) % self.condition_size == 0
        if self.use_mean:
            self.mean, self.scale = condition.chunk(2, dim=1)
        ...
        if self.use_quant:
            self.quant = quant
            self.quant = lower_bound(self.quant, 0.5)
    ...
    def get_condition(self):
        """return mean and scale"""
        if self.quant:
            return self.mean, self.quant, self.scale
        else:
            return self.mean, self.scale
    ...
    def quantize(self, input, mode, mean=None, quant=None):
        """Perturb or quantize a `Tensor` and optionally dequantize.
        ...
        if mode == "pass":
            return input
        if mode == 'RSGA':
            return random_quant(input, self.SGA, mean, quant=quant)
        if mode == 'RUN':
            return random_quant(input, noise_quant, mean, quant=quant)
        if mode == 'SGA':
        ...
        outputs = quantize(input, mode, mean, quant)

        if mode == "round" or mode == "UQ":
            outputs = self.dequantize(outputs, mean, quant)
        elif mode == "symbols":
            outputs = outputs.short()

        return outputs

    def dequantize(self, input, mean=None, quant=None):
        """Dequantize a `Tensor`.
    ...
    def _likelihood(self, input):
        ...
        upper = self._standardized_cumulative((values + t) / self.scale)
        lower = self._standardized_cumulative((values - t) / self.scale)
        likelihood = upper - lower

        return likelihood

    def forward(self, input, condition=None, quant=None):
        self._set_condition(condition, quant)

        output = self.quantize(
            input, self.quant_mode if self.training else "round", self.mean, self.quant)

        likelihood = self._likelihood(output)

        return output, likelihood, self.quant, self.mean, self.scale


class AdaptiveGaussianConditional(AdaptiveSymmetricConditional):
    """Conditional Gaussian entropy model.
...
__CONDITIONS__ = {"Gaussian": GaussianConditional, "Logistic": LogisticConditional, "Laplacian": LaplacianConditional,
                  ...
                  "AdaptiveGaussian": AdaptiveGaussianConditional}
